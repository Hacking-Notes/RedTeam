...
```Terminal 
hydra -l USERNAME -P PASSWORD.txt IP http-post-form "/PATH-URL/something.php:username=^USER^&password=^PASS^:INVALID-STATEMENT-HTML" -VV -T X
```
...
```Terminal 
hydra -l USERNMAE -P WORDLIST.txt MACHINE_IP -t X ssh
```
- ssh = Service
